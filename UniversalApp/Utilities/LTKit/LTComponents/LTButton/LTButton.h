...
//  继承自 QMUIButton,为了不在项目中导入QMUIKit框架,把内容复制到这里了
...
/// 控制图片在UIButton里的位置，默认为LTButtonImagePositionLeft
typedef NS_ENUM(NSUInteger, LTButtonImagePosition) {
    LTButtonImagePositionTop,             // imageView在titleLabel上面
    LTButtonImagePositionLeft,            // imageView在titleLabel左边
    LTButtonImagePositionBottom,          // imageView在titleLabel下面
    LTButtonImagePositionRight,           // imageView在titleLabel右边
};
 ...
 *  @warning LTButton 重新定义了 UIButton.titleEdgeInests、imageEdgeInsets、contentEdgeInsets 这三者的布局逻辑，sizeThatFits: 里会把 titleEdgeInests 和 imageEdgeInsets 也考虑在内（UIButton 不会），以使这三个接口的使用更符合直觉。
 */
...
/**
 * 设置按钮里图标和文字的相对位置，默认为LTButtonImagePositionLeft<br/>
 * 可配合imageEdgeInsets、titleEdgeInsets、contentHorizontalAlignment、contentVerticalAlignment使用
 */
@property(nonatomic, assign) LTButtonImagePosition imagePosition;
